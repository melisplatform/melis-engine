...
class MelisSiteTable extends MelisGenericTable
{
    ...
    public function getSites($env = '', $includeSite404Table = false)
    {
        ...
        if ($includeSite404Table) {
            $select->join(
                ...
                'melis_cms_site_domain.sdom_site_id = melis_cms_site.site_id',
                array('*'),
                $select::JOIN_LEFT
            )
                ->join(
                    'melis_cms_site_404',
                    'melis_cms_site_404.s404_site_id = melis_cms_site.site_id',
                    array('*'),
                    $select::JOIN_LEFT
                );
        } else {
    ...
    public function getSiteById($idSite, $env, $includeSite404Table = false)
    {
        ...
        $select->where('site_id =' . (int)$idSite);
    ...
    public function getSiteSavedPagesById($siteId)
    {
        ...
        $select->where('melis_cms_template.tpl_site_id =' . (int)$siteId);
